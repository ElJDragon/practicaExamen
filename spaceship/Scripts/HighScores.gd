	...
	if FileAccess.file_exists(SAVE_PATH):
		...
		if file:
			...
			if parse_result == OK:
				...
				print("⚠️ Error parsing high scores, creating new list")
				initialize_default_scores()
		else:
			print("⚠️ Could not open save file, creating new list")
			initialize_default_scores()
	else:
		print("📝 No save file found, creating new high scores list")
		initialize_default_scores()
...
func initialize_default_scores():
	...
	save_scores()
	print("✨ Default high scores initialized")
...
func is_high_score(score: int) -> bool:
	...
	# Verificar si es mayor que el puntaje más bajo
	return score > high_scores[MAX_SCORES - 1]["score"]

func get_rank_position(score: int) -> int:
	"""Obtener la posición que ocuparía el puntaje en el ranking (-1 si no califica)"""
	...
	# Si llegamos aquí y hay espacio, va al final
	if high_scores.size() < MAX_SCORES:
...
func add_high_score(player_name: String, score: int) -> int:
	"""Agregar un nuevo high score y retornar su posición"""
	var position = get_rank_position(score)
	
	if position == -1:
		print("⚠️ Score doesn't qualify for ranking")
		return -1
...
func clear_scores():
	"""Limpiar todos los scores (útil para debug/testing)"""
	initialize_default_scores()
	print("🗑️ High scores cleared and reset to defaults")
