...
var hud: Node2D  # HUD es un Node2D según HUD.gd
...
# Configuración de movimiento de enemigos - SIMPLIFICADO PARA DEBUG
var enemy_direction: int = 1
var enemy_move_timer: float = 0.0
var enemy_move_speed: float = 1.0  # Movimiento cada segundo para debug fácil
var enemy_drop_distance: float = 20.0
var enemy_move_distance: float = 20.0  # Movimiento más visible
...
func setup_game():
	...
	# CORREGIR LA POSICIÓN DEL JUGADOR
	...
	# Posicionar el HUD en una capa superior para evitar superposición
	...
	# Asegurar que el mensaje de Game Over esté oculto al inicio
	...
	# VERIFICACIÓN FINAL: Asegurar que solo hay elementos únicos
	var all_players = []
	var all_enemies = []
...
func create_enemies():
	...
	for row in ENEMY_ROWS:
		for col in ENEMY_COLS:
			...
			# Configurar tipo de enemigo según la fila
			if row == 0:
			...
			# DEBUG: Verificar que no hay enemigos en la misma posición
	...
	# Verificar duplicados y posiciones problemáticas
	detect_enemy_issues()
	...
	if enemy_move_timer >= enemy_move_speed:
		...
		if will_hit_edge:
			...
			for i in range(enemies.size()):
				var enemy = enemies[i]
				if is_instance_valid(enemy):  # Verificar que el enemigo es válido
					var old_pos = enemy.position
					enemy.position.x += move_amount
					moved_count += 1
					
					# Verificar si realmente se movió
					if abs(enemy.position.x - old_pos.x) < 0.1:  # No se movió
						static_enemies.append(i)
					
					# Debug para enemigos problemáticos
					if moved_count <= 3:
						print("Enemy ", moved_count, " moved from ", old_pos, " to ", enemy.position)
			
			# Reportar enemigos estáticos
...
func next_level():
	...
	# Resetear posición de enemigos y crear nueva formación
	enemy_direction = 1
	enemy_move_timer = 0.0
	enemy_shoot_timer = 0.0
...
func _on_player_hit():
	if not game_active:
		return  # Ignorar hits si el juego no está activo
...
func find_players_in_node(node: Node, player_list: Array):
	...
	for child in node.get_children():
		if child.get_script() and child.get_script().get_path() == "res://spaceship/scenes/Player.tscn":
			...
			# Detectar Players por sus señales características
			player_list.append(child)
...
func find_enemies_in_node(node: Node, enemy_list: Array):
	...
	for child in node.get_children():
		if child.get_script() and child.get_script().get_path() == "res://spaceship/Scripts/Enemy.gd":
			...
			# Detectar Enemies por su método característico
			enemy_list.append(child)
...
func detect_enemy_issues():
	"""Detectar enemigos duplicados, estáticos o problemáticos"""
...
func find_enemies_outside_container(node: Node, enemy_list: Array):
	...
	for child in node.get_children():
		# Si es un enemigo pero no está en el contenedor de enemigos
...
func find_all_enemies_recursive(node: Node, enemy_list: Array):
	"""Búsqueda recursiva de enemigos en toda la escena"""
	for child in node.get_children():
