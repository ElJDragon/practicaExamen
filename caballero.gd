...
func create_speech_bubble():
	...
	# Posicionar el globo arriba del caballero
	...
	# Agregar fondo al label
	var style_box = StyleBoxFlat.new()
	style_box.bg_color = Color(1, 1, 1, 0.95)
	style_box.border_color = Color(0, 0, 0, 0.8)
...
func _on_body_entered(body):
	if body.name == "Mono" or body.name == "PJ_Principal":
		print("✅ Jugador cerca del caballero")
...
func _on_body_exited(body):
	if body.name == "Mono" or body.name == "PJ_Principal":
		print("❌ Jugador se alejó del caballero")
...
func _process(delta):
	if player_in_range and (Input.is_action_just_pressed("ui_accept") or Input.is_key_pressed(KEY_E)):
		...
		if player_reference:
			...
			print("✅ Posición del jugador guardada (caballero):", Global.posicion_mono)
		else:
