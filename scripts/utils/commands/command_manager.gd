# ============================================================================
# COMMAND MANAGER
# ============================================================================
# Central command manager with undo/redo support
#
# DESIGN PATTERNS: Command Pattern + Memento Pattern
# SOLID: [SRP] Single Responsibility: Manage command history
# ============================================================================

class_name CommandManager
extends Node

## Max commands stored in history
const MAX_HISTORY_SIZE = 50

var _command_history: Array = []
var _current_index: int = -1

signal command_executed(command: Command)
signal command_undone(command: Command)
signal command_redone(command: Command)

## Execute a command and add it to history
func execute_command(command: Command) -> void:
	command.execute()
	
	# If there are commands after the current index, remove them
	if _current_index < _command_history.size() - 1:
		_command_history = _command_history.slice(0, _current_index + 1)
	
	_command_history.append(command)
	_current_index += 1
	
	# Limit history size
	if _command_history.size() > MAX_HISTORY_SIZE:
		_command_history.pop_front()
		_current_index -= 1
	
	command_executed.emit(command)
	print(" Command executed: " + command.get_description())

## Undo the last command
func undo() -> bool:
	if not can_undo():
		print(" No commands to undo")
		return false
	
	var command = _command_history[_current_index]
	if command.is_reversible():
		command.undo()
		_current_index -= 1
		command_undone.emit(command)
		print(" Command undone: " + command.get_description())
		return true
	else:
		print(" Command not reversible: " + command.get_description())
		return false

## Redo the previously undone command
func redo() -> bool:
	if not can_redo():
		print(" No commands to redo")
		return false
	
	_current_index += 1
	var command = _command_history[_current_index]
	command.execute()
	command_redone.emit(command)
	print(" Command redone: " + command.get_description())
	return true

func can_undo() -> bool:
	return _current_index >= 0

func can_redo() -> bool:
	return _current_index < _command_history.size() - 1

func clear_history() -> void:
	_command_history.clear()
	_current_index = -1
	print(" Command history cleared")

func get_history_size() -> int:
	return _command_history.size()

func get_current_index() -> int:
	return _current_index
