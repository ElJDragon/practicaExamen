...
# Anfitrión de la trivia: crea sala, mantiene lista de jugadores, distribuye preguntas y calcula puntajes.
...
func _ready():
	...
	# conectar señales (si el editor no lo hizo)
...
# Lógica para cuando un cliente se une (esto debe llamarse desde callback de red)
...
func start_game():
	# Asignar preguntas aleatorias a cada jugador según las categorias y preguntas_per_player
	questions_map.clear()
	for pid in players.keys():
		var assigned := []
		...
		for c in categories:
			...
			for q in qs:
				assigned.append(q)
		# si faltan, rellenar con preguntas al azar de la primera categoría
...
# Este método será llamado por la red cuando el jugador envíe su resultado parcial
@rpc
...
# Callbacks para networking (placeholders): deberás conectarlos a los eventos reales del Peer
func _on_network_peer_connected(id):
	...
	emit_signal("player_list_updated", get_player_list())
	# si el host local también quiere aparecer, usar id 1? (host id es 1 en ENet)
